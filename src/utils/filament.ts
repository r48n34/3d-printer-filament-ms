...
export const getPrintTotal = (record: Pick<PrintRecord, "quantity" | "gramsPerItem">) =>
...
export const getEstimatedPrintCost = (
  ...
  if (spool.purchasePrice === undefined || spool.initialWeightG <= 0) return undefined;
  return (
    Math.round(((spool.purchasePrice * getPrintTotal(record)) / spool.initialWeightG) * 10000) /
    10000
  );
};
...
export const getLedgerEntries = (
  ...
  [
    ...
    const byCreation = left.record.createdAt.localeCompare(right.record.createdAt);
...
export const getMaterialName = (spool: Spool) =>
  spool.material === "Other" ? spool.customMaterial || "Other" : spool.material || "Unspecified";
