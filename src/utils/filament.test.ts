...
const makeAdjustment = (overrides: Partial<AdjustmentRecord> = {}): AdjustmentRecord => ({
  id: "adjustment-1",
  spoolId: spool.id,
  kind: "add",
  amountG: 10,
...
describe("filament calculations", () => {
  ...
  it("estimates material cost from spool price and starting weight", () => {
    const print = makePrint();
    expect(getEstimatedPrintCost({ initialWeightG: 1000, purchasePrice: 200 }, print)).toBe(5.2);
    expect(getEstimatedPrintCost({ initialWeightG: 1000 }, print)).toBeUndefined();
  });
  ...
  it("uses creation time and ID to order equal-time entries deterministically", () => {
    const entries = getLedgerEntries(
      [makePrint({ id: "z-print", createdAt: "2026-01-01T12:00:00.000Z" })],
      [
        makeAdjustment({
          id: "a-adjustment",
    ...
    expect(entries.map(({ record }) => record.id)).toEqual(["a-adjustment", "z-print"]);
  });

  it("allows a negative calculated balance", () => {
    expect(calculateBalance(spool, [makePrint({ quantity: 1, gramsPerItem: 120 })], [])).toBe(-20);
  });
});
