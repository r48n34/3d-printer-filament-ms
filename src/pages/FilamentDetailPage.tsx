...
import type { AdjustmentRecord, LedgerEntry, PrintRecord, Spool } from "../types";
import { formatDate, formatDateTime, formatGrams, formatMoney } from "../utils/format";
import {
...
export function FilamentDetailPage() {
  ...
  const [editingAdjustment, setEditingAdjustment] = useState<AdjustmentRecord>();
    ...
    return (
      <Stack gap="lg">
        ...
        <Alert color="orange" title="Filament not found">
          This spool may have been removed or the link is no longer valid.
        </Alert>
      </Stack>
    );
  }
  ...
  const spoolAdjustments = adjustments.filter((record) => record.spoolId === spool.id);
  ...
  const printedWeight = spoolPrints.reduce((sum, record) => sum + getPrintTotal(record), 0);
  const estimatedCost =
    spool.purchasePrice === undefined
      ? undefined
      : spoolPrints.reduce((sum, record) => sum + (getEstimatedPrintCost(spool, record) ?? 0), 0);
  ...
  const removeEntry = (entry: LedgerEntry) => {
    const label = entry.type === "print" ? entry.record.projectName : entry.record.reason;
    modals.openConfirmModal({
      ...
      onConfirm: async () => {
        if (entry.type === "print") await db.prints.delete(entry.record.id);
        else await db.adjustments.delete(entry.record.id);
        notifications.show({ color: "teal", message: "Activity deleted." });
      },
    });
  };
  ...
  return (
    <Stack gap="xl">
      ...
      <PageHeader
        ...
        actions={
          <Group gap="sm">
            <Button variant="default" leftSection={<IconEdit size={17} />} onClick={editModal.open}>
              Edit details
            </Button>
            <Button
              variant="light"
      ...
      {spool.archivedAt ? (
        <Alert color="gray" title="This spool is archived">
          <Group justify="space-between" gap="md">
            <Text size="sm">Restore it before adding prints or stock adjustments.</Text>
            <Button
              size="xs"
              variant="light"
        ...
        <Group justify="space-between" mb="sm">
          <div>
            ...
            <Text size="xs" c="dimmed">
              {formatGrams(balance)} remaining from {formatGrams(spool.initialWeightG)} starting
              weight
            </Text>
          </div>
          <Badge variant="light" color={balance < 0 ? "red" : progress < 20 ? "orange" : "copper"}>
            {balance < 0 ? "Below zero" : `${Math.round(progress)}% left`}
          </Badge>
        </Group>
        <Progress
          value={progress}
          size="lg"
          radius="xl"
          color={balance < 0 ? "red" : progress < 20 ? "orange" : "copper"}
      ...
      <Grid>
        <Grid.Col span={{ base: 12, lg: 4 }}>
          <Card withBorder radius="lg" p="xl" h="100%">
            <Group justify="space-between" mb="lg">
              ...
              <Tooltip label={spool.archivedAt ? "Restore spool" : "Archive spool"}>
                <ActionIcon
                  variant="light"
                  color="gray"
                  onClick={toggleArchive}
                  aria-label={spool.archivedAt ? "Restore spool" : "Archive spool"}
                >
                  {spool.archivedAt ? <IconRestore size={18} /> : <IconArchive size={18} />}
                </ActionIcon>
              </Tooltip>
            </Group>
            <Stack gap="md">
              ...
              <DetailItem
                ...
                value={
                  spool.purchasePrice !== undefined
                    ? formatMoney(spool.purchasePrice, spool.purchaseCurrency)
                    : "Not provided"
                }
                note={
                  spool.purchasePrice !== undefined
        ...
        <Grid.Col span={{ base: 12, lg: 8 }}>
          <Card withBorder radius="lg" p="xl" h="100%">
            <Group justify="space-between" mb="lg">
              <div>
                <Title order={2}>History & related activity</Title>
                <Text size="sm" c="dimmed">
                  Prints, waste, measurements, and stock changes
                </Text>
              </div>
              <Button variant="subtle" onClick={() => navigate(`/history?spool=${spool.id}`)}>
            ...
            {entries.length ? (
              ...
              <Paper className="detail-empty" radius="lg" p="xl">
                <ThemeIcon variant="light" color="copper" radius="xl" size={42}>
                  <IconPrinter size={21} />
                </ThemeIcon>
                <Text fw={700} mt="sm">
      ...
      <SpoolFormModal opened={editOpened} onClose={editModal.close} spool={spool} />
      <PrintFormModal
        opened={printOpened}
    ...
    <Paper withBorder radius="lg" p="lg">
      <Group justify="space-between" align="flex-start" wrap="nowrap">
        <div>
          ...
          <Text fz={24} fw={800} mt={5} c={color === "red" ? "red" : undefined}>
            {value}
          </Text>
...
function ActivityRow({
  ...
  const printCost = isPrint ? getEstimatedPrintCost(spool, entry.record) : undefined;
  ...
  return (
    <Group className="detail-activity-row" justify="space-between" wrap="nowrap">
      <Group gap="sm" wrap="nowrap" className="detail-activity-main">
        <ThemeIcon variant="light" color={isPrint ? "copper" : "blue"} radius="xl">
          {isPrint ? <IconPrinter size={17} /> : <IconAdjustmentsHorizontal size={17} />}
        </ThemeIcon>
        <div className="detail-activity-copy">
          <Group gap="xs">
            ...
            <Badge size="xs" variant="light" color={isPrint ? "orange" : "blue"}>
