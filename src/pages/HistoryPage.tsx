...
import { getEstimatedPrintCost, getLedgerEntries, getPrintTotal } from "../utils/filament";
...
export function HistoryPage() {
  ...
  const [spoolId, setSpoolId] = useState<string | null>(searchParams.get("spool"));
  ...
  const entries = getLedgerEntries(prints, adjustments)
    ...
    .filter((entry) => {
      ...
      if (dateRange[0] && dayjs(entry.occurredAt).isBefore(dayjs(dateRange[0]).startOf("day")))
        return false;
      if (dateRange[1] && dayjs(entry.occurredAt).isAfter(dayjs(dateRange[1]).endOf("day")))
        return false;
      const term = search.trim().toLowerCase();
      if (!term) return true;
      const text = entry.type === "print" ? entry.record.projectName : entry.record.reason;
  ...
  const remove = (entry: LedgerEntry) => {
    ...
    modals.openConfirmModal({
      ...
      onConfirm: async () => {
        ...
        notifications.show({ color: "teal", message: "History entry deleted." });
      },
    });
  };
  ...
  return (
    <Stack gap="xl">
        ...
        actions={
          <Group gap="sm">
            <Button
              ...
              disabled={!spools.some(({ archivedAt }) => !archivedAt)}
            >
              Adjust stock
            </Button>
            <Button
      ...
      <Paper withBorder radius="lg" p="md">
        <Group align="flex-end" gap="sm">
          ...
          <Select
            ...
            clearable
            searchable
            data={spools.map((spool) => ({ value: spool.id, label: spool.name }))}
            value={spoolId}
            onChange={setSpoolId}
          />
          <DatePickerInput
          ...
          />
          {search || type || spoolId || dateRange[0] || dateRange[1] ? (
            <Button
              variant="subtle"
              color="gray"
              onClick={() => {
      ...
      {entries.length ? (
        <>
          <Card withBorder radius="lg" p={0} className="desktop-history">
              ...
              <Table verticalSpacing="md" horizontalSpacing="lg" highlightOnHover>
                <Table.Thead>
                  <Table.Tr>
                    ...
                    <Table.Th ta="right">Est. material cost</Table.Th>
                    <Table.Th w={48}>
                ...
                <Table.Tbody>
                  {entries.map((entry) => (
                    <HistoryRow
                      ...
                      entry={entry}
                      spool={spools.find(({ id }) => id === entry.record.spoolId)}
                      onEdit={() =>
                        entry.type === "print"
          ...
          <Stack className="mobile-history" gap="sm">
            {entries.map((entry) => (
              <HistoryCard
                ...
                spool={spools.find(({ id }) => id === entry.record.spoolId)}
                onEdit={() =>
                  entry.type === "print" ? openPrint(entry.record) : openAdjustment(entry.record)
                }
                onDelete={() => remove(entry)}
              />
            ))}
        ...
        <EmptyState
          title={prints.length || adjustments.length ? "No matching activity" : "No history yet"}
          description={
...
function entryDetails(entry: LedgerEntry) {
  ...
  const symbol = entry.record.kind === "add" ? "+" : entry.record.kind === "remove" ? "−" : "=";
  return {
    ...
    <Menu position="bottom-end" shadow="md">
      <Menu.Target>
        <Tooltip label="Entry actions">
          <ActionIcon variant="subtle" color="gray" aria-label={`Actions for ${entry.type} entry`}>
            <IconDots size={18} />
          </ActionIcon>
        </Tooltip>
      </Menu.Target>
...
function HistoryRow({
  ...
  const estimatedCost =
    entry.type === "print" && spool ? getEstimatedPrintCost(spool, entry.record) : undefined;
  return (
    <Table.Tr>
      <Table.Td>
      ...
      <Table.Td ta="right">
        <Text fw={700} c={entry.type === "print" ? "orange.8" : undefined}>
          {details.change}
        </Text>
      </Table.Td>
      <Table.Td ta="right">
        <Text size="sm" fw={600} c={estimatedCost === undefined ? "dimmed" : undefined}>
          {entry.type === "print"
        ...
        <div>
          ...
          {entry.type === "print" ? (
            <Text size="xs" c="dimmed" mt={5}>
              ...
              <Text span fw={700} c={estimatedCost === undefined ? "dimmed" : undefined}>
                {estimatedCost !== undefined
                  ? formatMoney(estimatedCost, spool?.purchaseCurrency)
                  : "Not priced"}
              </Text>
