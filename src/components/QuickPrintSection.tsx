...
import { calculateBalance, getEstimatedPrintCost, getPrintTotal } from "../utils/filament";
...
export function QuickPrintSection({
  ...
  const form = useForm<QuickPrintValues>({
    ...
    validate: {
      ...
      projectName: (value) => (value.trim() ? null : "Enter what you printed"),
      quantity: (value) =>
        typeof value === "number" && Number.isInteger(value) && value > 0
          ? null
          : "Use a whole number",
      gramsPerItem: (value) =>
        typeof value === "number" && value > 0 ? null : "Enter grams per item",
    },
  });
  ...
  const selectedSpool = activeSpools.find(({ id }) => id === form.values.spoolId);
  ...
  const projectedBalance =
    selectedSpool && candidate
      ? calculateBalance(
          selectedSpool,
          [...prints.filter(({ spoolId }) => spoolId === selectedSpool.id), candidate],
          adjustments.filter(({ spoolId }) => spoolId === selectedSpool.id),
        )
      : 0;
  const estimatedCost =
    selectedSpool && candidate ? getEstimatedPrintCost(selectedSpool, candidate) : undefined;
  ...
  const submit = form.onSubmit((values) => {
    if (projectedBalance < 0) {
      modals.openConfirmModal({
        ...
        children: (
          <Text size="sm">
            The new balance will be {formatGrams(projectedBalance)}. Add this print anyway?
          </Text>
        ),
  ...
  return (
    <Paper className="quick-print-section" radius="xl" p={{ base: "lg", sm: "xl" }}>
      <Stack gap="lg">
        <Group justify="space-between" align="flex-start">
          <Group gap="md" wrap="nowrap">
            ...
            <div>
              <Title order={2}>Add printing history</Title>
              <Text size="sm" c="dimmed" mt={3}>
                Log a completed print and update the spool balance instantly.
              </Text>
            </div>
          </Group>
          {total > 0 ? (
            <div className="quick-total">
              ...
              <Text size="xs" c="dimmed" fw={700} tt="uppercase" mt={6}>
                Est. material cost
              </Text>
              <Text fw={800} fz="lg">
                {estimatedCost !== undefined
                  ? formatMoney(estimatedCost, selectedSpool?.purchaseCurrency)
                  : "Not priced"}
              </Text>
            </div>
          ) : null}
          ...
          <form onSubmit={submit}>
            <Grid align="flex-end" gap="sm">
              ...
              <Grid.Col span={{ base: 12, md: 2 }}>
                <Button type="submit" fullWidth leftSection={<IconPlus size={17} />}>
                  Add history
                </Button>
              </Grid.Col>
            </Grid>
            {projectedBalance < 0 && total > 0 ? (
              <Alert color="red" variant="light" mt="md" icon={<IconAlertTriangle size={17} />}>
                This would leave {selectedSpool?.name} at {formatGrams(projectedBalance)}.
              </Alert>
            ) : null}
            <Text size="xs" c="dimmed" mt="sm">
              ...
              Saved with the current date and time; you can add notes or correct it from History.
            </Text>
          </form>
        ) : (
          <Alert color="copper" variant="white" title="Add a spool first">
            <Group justify="space-between" gap="md">
              <Text size="sm">Printing history needs a filament spool to deduct from.</Text>
              <Button size="xs" variant="light" onClick={onAddSpool}>
                Add spool
              </Button>
            </Group>
          </Alert>
