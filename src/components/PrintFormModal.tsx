...
export function PrintFormModal({
  ...
  const form = useForm<PrintFormValues>({
    ...
    validate: {
      ...
      projectName: (value) => (value.trim() ? null : "Enter a project name"),
      quantity: (value) =>
        typeof value === "number" && Number.isInteger(value) && value > 0
          ? null
          : "Quantity must be a positive whole number",
      gramsPerItem: (value) =>
        typeof value === "number" && value > 0 ? null : "Enter grams used per item",
      printedAt: (value) => (dayjs(value).isValid() ? null : "Enter a valid date and time"),
    },
  });
  ...
  const save = form.onSubmit((values) => {
    if (projectedBalance < 0) {
      modals.openConfirmModal({
        ...
        children: (
          <Text size="sm">
            The projected balance is {formatGrams(projectedBalance)}. Save this record anyway?
          </Text>
        ),
        ...
        <Stack gap="md">
          ...
          <Grid>
            ...
            <Grid.Col span={{ base: 6, sm: 3 }}>
              ...
              <Text className="calculated-value">{formatGrams(total)}</Text>
            </Grid.Col>
            <Grid.Col span={{ base: 6, sm: 3 }}>
              <Text size="sm" fw={500} mb={7}>
                Est. material cost
          ...
          {projectedBalance < 0 ? (
            <Alert color="red" icon={<IconAlertTriangle size={18} />} title="Negative balance">
              This record would leave {selectedSpool?.name} at {formatGrams(projectedBalance)}. You
              can still save it after confirmation.
            </Alert>
          ) : null}
          <Group justify="flex-end">
            ...
            <Button type="submit">{record ? "Save changes" : "Record print"}</Button>
          </Group>
        </Stack>
      </form>
    </Modal>
