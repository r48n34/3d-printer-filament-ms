...
export function SpoolCard({ spool, balance, onEdit, onArchive, onOpen }: SpoolCardProps) {
        ...
        <Group justify="space-between" align="flex-start" wrap="nowrap">
          <Group gap="sm" wrap="nowrap">
            <ThemeIcon
              ...
              variant="light"
              color="copper"
              style={spool.color ? { color: spool.color } : undefined}
            >
              <IconDisc size={23} />
            </ThemeIcon>
            <div>
          ...
          <Menu position="bottom-end" shadow="md">
            <Menu.Target>
              <Tooltip label="Spool actions">
                <ActionIcon variant="subtle" color="gray" aria-label={`Actions for ${spool.name}`}>
                  <IconDots size={19} />
                </ActionIcon>
              </Tooltip>
            </Menu.Target>
            <Menu.Dropdown>
              ...
              <Menu.Item
                leftSection={
                  spool.archivedAt ? <IconRestore size={16} /> : <IconArchive size={16} />
                }
                onClick={(event) => {
                  event.stopPropagation();
                  onArchive();
              ...
              >
                {spool.archivedAt ? "Restore spool" : "Archive spool"}
              </Menu.Item>
            </Menu.Dropdown>
          </Menu>
        </Group>
